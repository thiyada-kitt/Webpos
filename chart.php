                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" id="datepicker2" name="date_2" value=""  data-date-format="yyyy-mm-dd" required>
                                    ...
                                    <div class="col-md-2">
                                         ...
                                         <div align="left">
                    <input type="submit" name="btndatefilter" class="btn btn-success" value="Filter By Date" />
...
<!--                <canvas id="areaChart" style="height:250px"></canvas>-->
   ...
   <?php       
$select=$pdo->prepare("select product_name,sum(qty) as qty from tbl_invoice_details WHERE order_date BETWEEN :fromdate AND :todate group by product_id");
...
$sql="SELECT sum(total) AS total , sum(subtotal) as stotal,count(invoice_id) as inv FROM tbl_invoice  ";
...
//$select=$pdo->prepare("select product_name,sum(qty) as qty from tbl_invoice_details group by product_id");
////  SELECT Items, sum(Qty)  FROM Bill_items where Date between  '"+strOutput1+"' AND  '"+strOutput21+"' group by Dish_ID         
//
//
//    $select->execute();
...
$sql="SELECT count(pname) as p FROM tbl_product";
            $stmt = $pdo->prepare($sql);
...
$sql="SELECT count(category) as cate FROM tbl_category";
            $stmt = $pdo->prepare($sql);
...
<div class="col-md-6">
   ...
   <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title">Best Selling Menu</h3>
...
$sql = "SELECT product_id,product_name ,price,sum(qty) as qty , sum(price*qty) as total FROM tbl_invoice_details group by product_id order by sum(qty) DESC LIMIT 30";
    $stmt = $pdo->prepare($sql);
